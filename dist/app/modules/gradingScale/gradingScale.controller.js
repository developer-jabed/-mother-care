...
const createGradingScale = catchAsync(async (request, reply) => {
    const result = await GradingScaleService.createGradingScale(request.body);
...
const getAllGradingScales = catchAsync(async (request, reply) => {
    const filters = pick(request.query, gradingScaleFilterableFields);
...
const getGradingScalesByAcademicYear = catchAsync(async (request, reply) => {
    ...
    const result = await GradingScaleService.getGradingScalesByAcademicYear(Number(academicYearId));
    return sendResponse(reply, {
        statusCode: httpStatus.OK,
        success: true,
        message: 'Grading scales retrieved successfully',
...
const updateGradingScale = catchAsync(async (request, reply) => {
    const { id } = request.params;
    const result = await GradingScaleService.updateGradingScale(Number(id), request.body);
    return sendResponse(reply, {
        statusCode: httpStatus.OK,
        success: true,
...
//# sourceMappingURL=gradingScale.controller.js.map