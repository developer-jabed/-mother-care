...
const createExamType = catchAsync(async (request, reply) => {
    const result = await ExamTypeService.createExamType(request.body);
...
const getAllExamTypes = catchAsync(async (request, reply) => {
    const filters = pick(request.query, examTypeSearchableFields);
...
const updateExamType = catchAsync(async (request, reply) => {
    const { id } = request.params;
    const result = await ExamTypeService.updateExamType(Number(id), request.body);
    return sendResponse(reply, {
        statusCode: httpStatus.OK,
        success: true,
