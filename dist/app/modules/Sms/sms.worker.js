...
const smsWorker = new Worker('result-sms', // must match the queue name in queue.ts exactly
async (job) => {
    ...
    try {
        ...
        await updateSmsLog(studentEnrollmentId, examId, isRetryable ? 'PENDING' : 'FAILED', undefined, error.message);
        if (!isRetryable) {
