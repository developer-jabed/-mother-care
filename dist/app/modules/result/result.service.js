...
const buildResultComputation = async (examId, details) => {
    ...
    if (missing.length > 0) {
        throw new ApiError(httpStatus.BAD_REQUEST, `Invalid subjectId(s): ${missing.join(', ')}`);
    }
    ...
    if (gradingScales.length === 0) {
        throw new ApiError(httpStatus.BAD_REQUEST, 'No grading scale configured for this academic year');
    }
    ...
    const computedDetails = details.map(detail => {
        ...
        if (rawTotal > subject.fullMarks) {
            throw new ApiError(httpStatus.BAD_REQUEST, `Marks for subject "${subject.name}" (${rawTotal}) exceed full marks (${subject.fullMarks})`);
        }
    ...
    const { grade: overallGrade, gradePoint: overallGradePoint } = resolveGrade(overallPercentage, gradingScales);
    return {
        computedDetails,
        overallObtained,
        overallPercentage,
...
const createResult = async (payload) => {
    ...
    if (existing) {
        throw new ApiError(httpStatus.CONFLICT, 'A result already exists for this student and exam. Use update instead.');
    }
    const { computedDetails, overallObtained, overallPercentage, overallGrade, overallGradePoint, } = await buildResultComputation(examId, details);
...
const getAllResults = async (filters, paginationOptions) => {
...
const updateResult = async (id, payload) => {
    ...
    if (existing.isPublished) {
        throw new ApiError(httpStatus.BAD_REQUEST, 'Cannot modify a published result. Unpublish it first.');
    }
    const result = await prisma.$transaction(async (tx) => {
        if (payload.details && payload.details.length > 0) {
            const { computedDetails, overallObtained, overallPercentage, overallGrade, overallGradePoint, } = await buildResultComputation(existing.examId, payload.details);
...
const calculatePositions = async (examId) => {
    ...
    await prisma.$transaction(results.map((result, index) => prisma.result.update({
        where: { id: result.id },
...
const deleteResult = async (id) => {
    const existing = await getSingleResult(id);
    if (existing.isPublished) {
        throw new ApiError(httpStatus.BAD_REQUEST, 'Cannot delete a published result. Unpublish it first.');
    }
