...
if (redisUrl) {
    ...
    const resultCardWorker = new Worker<IResultCardJobData, IResultCardJobResult>(
        ...
        async (job: Job<IResultCardJobData>) => {
            ...
            console.log(`📄 [${job.id}] Generating result cards — class ${classId}, section ${sectionId}`);
            ...
            if (!result.pdfBuffer) {
                throw new Error(`All ${result.totalStudents} result cards failed. Reasons: ${JSON.stringify(result.failed)}`);
            }
            ...
            if (result.failed.length > 0) {
                ...
                console.log(`✅ [${job.id}] All ${result.totalStudents} result cards generated successfully`);
            }
    ...
    resultCardWorker.on('ready', () => console.log('📄 Result card worker connected (Redis available)'));
    resultCardWorker.on('error', (err) => console.error('🔥 Result card worker error:', err));
    resultCardWorker.on('completed', (job) => console.log(`🎉 Result card job ${job.id} completed`));
    resultCardWorker.on('failed', (job, err) => console.error(`💀 Result card job ${job?.id} failed permanently:`, err.message));
} else {
    console.warn('⚠️ BULLMQ_REDIS_URL not set — result card worker not started (direct-generation fallback only)');
}