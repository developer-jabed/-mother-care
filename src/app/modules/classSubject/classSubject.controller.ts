...
const createClassSubject = catchAsync(async (request, reply) => {
    const result = await ClassSubjectService.createClassSubject(request.body as any);
...
const getAllClassSubjects = catchAsync(async (request, reply) => {
    const filters = pick(request.query as any, classSubjectFilterableFields);
...
const getSubjectsByClassAndSection = catchAsync(async (request, reply) => {
    ...
    const result = await ClassSubjectService.getSubjectsByClassAndSection(
        Number(classId),
        Number(sectionId)
    );
