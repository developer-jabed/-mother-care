...
const getAllExams = async (
  ...
  const { searchTerm, academicYearId, ...filterData } = filters;

  const andConditions: Prisma.ExamWhereInput[] = [];

  // ── Always scope to current academic year (unless a specific year is requested)
  if (academicYearId) {
    andConditions.push({ academicYearId: Number(academicYearId) });
  } else {
    // Get current academic year and filter by it
